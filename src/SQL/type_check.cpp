...
{
    return type == TYPE_NULL || type == TYPE_INT || type == TYPE_STRING ||
           type == TYPE_BOOL;
}

static inline bool find_column(TypedTableSchema schema, StringView name,
                               Type *type)
{
    for (UZ i = 0; i < schema.column_names.count; ++i)
    {
        if (schema.column_names[i].has_value() &&
            schema.column_names[i].value == name)
        {
...
static void error_on(TypingContext *ctx, Token token, String message)
{
    ...
    ctx->error = ErrorWithSourceLocation{.message = message,
...
static inline bool type_check_ir_instruction(U32 ip, CompiledQuery *ir_emitter,
                                             TypingContext *ctx)
{
    ...
    case IRInstructionOperator_Eq:
    {
        ...
        if (!types_are_equal(lhs_type, rhs_type))
        {
            ...
            String message = String::format(
                    ctx->allocator,
                    "cannot compare lhs of type '%s' to rhs of type '%s'",
    ...
    case IRInstructionOperator_FetchTable:
    {
        Triple<String, StringView, TableSchema *> operands =
                operands_of_FetchTable(ir_emitter, ip);
    ...
    case IRInstructionOperator_FetchColumn:
    {
        Triple<String, U32, StringView> operands =
                operands_of_FetchColumn(ir_emitter, ip);

        U32 table_type_ip = operands.op2;
        StringView column_name = operands.op3;

        TypedTableSchema table_schema =
                ctx->table_types.get(table_type_ip).get();
    ...
    case IRInstructionOperator_EmitQuery:
    {
        ...
        List<Optional<String>> column_names =
                List<Optional<String>>::alloc(ctx->allocator);
        List<Type> column_types = List<Type>::alloc(ctx->allocator);

        for (UZ i = ctx->emitted_columns.count - columns_to_emit_count;
             i < ctx->emitted_columns.count; ++i)
        {
            U32 emit_column_ip = ctx->emitted_columns[i];
            Triple<U32, U32, StringView> emit_column_ops =
                    operands_of_EmitColumn(ir_emitter, emit_column_ip);

            Type column_type =
                    ctx->ir_instruction_types.get(emit_column_ops.op2).get();
    ...
    case IRInstructionOperator_InsertColumn:
    {
        Tuple<StringView, U32> operands =
                operands_of_InsertColumn(ir_emitter, ip);
    ...
    case IRInstructionOperator_CommitInsert:
    {
        ...
        for (UZ i = 0; i < ctx->insert_column_names.count; ++i)
        {
            ...
            OK_ASSERT(find_column(table_schema, column_name,
            ...
            if (!types_are_equal(column_type, expected_column_type))
            {
                ...
                String message = String::format(
                        ...
                        type_name(expected_column_type),
    ...
    case IRInstructionOperator_UpdateColumn:
    {
        Triple<U32, U32, StringView> operands =
                operands_of_UpdateColumn(ir_emitter, ip);

        TypedTableSchema table_schema =
                ctx->table_types.get(operands.op1).get();

        Type column_type = ctx->ir_instruction_types.get(operands.op2).get();

        Type expected_column_type;
        OK_ASSERT(
                find_column(table_schema, operands.op3, &expected_column_type));
    ...
    {
        Triple<ok::String, ok::StringView, S64> operands =
                operands_of_Call(ir_emitter, ip);
        StringView fn_name = operands.op2;
        S64 argument_count = operands.op3;

        Optional<FunctionSignature> fn_signature_opt =
                ctx->function_signatures.get(fn_name);
        if (!fn_signature_opt.has_value())
        {
            Token token = ir_emitter->tokens[ip];
            String message = String::format(
                    ctx->allocator, "undefined function '" OK_SV_FMT "'",
                    OK_SV_ARG(fn_name));
            error_on(ctx, token, message);
            return false;
        }
        ...
        if ((S64) signature.parameter_types.count != argument_count)
        {
            ...
            String message = String::format(
                    ctx->allocator,
                    "function '" OK_SV_FMT
                    "' expects %zu arguments, but %ld provided",
                    OK_SV_ARG(fn_name), signature.parameter_types.count,
                    argument_count);
            error_on(ctx, token, message);
            return false;
        }
        ...
        for (UZ i = 0; i < ctx->call_args.count; ++i)
        {
            ...
            if (arg_type != param_type)
            {
                ...
                String message = String::format(
                        ctx->allocator,
                        "expected call argument at index %zu to be of type "
                        "'%s', but got a value of type '%s' instead",
...
TypingContext::TypingContext(Allocator *allocator, StringView source) :
    allocator{allocator}, source{source}
{
    ir_instruction_types = Table<U32, Type>::alloc(allocator);
    table_types = Table<U32, TypedTableSchema>::alloc(allocator);
    function_signatures =
            Table<StringView, FunctionSignature>::alloc(allocator);
...
#define X(fn_name, ret_type, ...)                                              \
    do {                                                                       \
        Type return_type = cpp_to_tt<ret_type>();                              \
        Slice<Type> parameter_types =                                          \
                cpp_to_tt_multiple<__VA_ARGS__>(allocator);                    \
        function_signatures.put(                                               \
                ok::StringView{#fn_name},                                      \
                FunctionSignature{return_type, parameter_types});              \
    }                                                                          \
    while (false);
