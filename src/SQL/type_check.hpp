...
struct TypedCompiledQuery
{
    ...
    Table<U32, TypedTableSchema>
            table_types; ///< Mapping of instruction IDs to their resulting
                         ///< table schemas.
};
...
struct TypingContext
{
    ...
    Table<U32, Type> ir_instruction_types; ///< Mapping of instruction IDs to
                                           ///< their result types.
    Table<U32, TypedTableSchema>
            table_types; ///< Mapping of instruction IDs to table schemas.
    Table<StringView, FunctionSignature>
            function_signatures; ///< Mapping of function names to signatures.
    List<U32> emitted_columns; ///< Pending IDs of values emitted as columns of
                               ///< a query.
    List<U32> call_args; ///< IDs of values used as function arguments.
    List<StringView>
            insert_column_names; ///< Pending column names for insertion.
    List<U32> insert_column_values; ///< Pending value instruction IDs for
                                    ///< insertion.
    StringView source; ///< The original SQL source (for error reporting).
    Optional<ErrorWithSourceLocation>
            error; ///< Error information if type checking failed.
};
