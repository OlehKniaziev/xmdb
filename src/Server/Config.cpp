...
Config Config::parse(int argc, char **argv)
{
    ...
    arg_parser.string("protocol", &protocol_str, "Network protocol to use",
                      default_protocol);
    arg_parser.integer("port", &port_s64, "Port to start the server on",
                       default_port);
    ...
    if (!arg_parser.parse())
    {
        ...
        dief("Failed to parse command line arguments: " OK_SV_FMT,
