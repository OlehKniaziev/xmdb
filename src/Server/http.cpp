...
DECLARE_HANDLER(connect_handler)
{
    ...
    if (!WebJsonObjectGetStringView(&json_obj, WEB_SV_LIT("username"),
                                    &username))
    {
        FAIL(BAD_REQUEST, "'username' field not present");
    }
    if (!WebJsonObjectGetStringView(&json_obj, WEB_SV_LIT("password_hash"),
                                    &password_hash_hex))
    {
    ...
    ok::StringView password_hash_hex_sv = {
            (const char *) password_hash_hex.Items, password_hash_hex.Count};

    ok::Optional<ok::Slice<U8>> password_hash_opt =
            xmdb::from_hex_string(&ctx_arena, password_hash_hex_sv);
    ...
    for (xmdb::DBDescriptor *db = shared_db_pool->db_descriptors; db != nullptr;
         db = db->next)
    {
    ...
    ok::StringView username_sv = {(const char *) username.Items,
                                  username.Count};
    ...
    // FIXME(oleh): Replace this password hash check against the password by a
    // check againts a computed hash.
...
DECLARE_HANDLER(run_query_handler)
{
    ...
    if (!WebJsonObjectGetNumber(&json_obj, WEB_SV_LIT("connection_id"),
                                &connection_id_f64))
    {
        FAIL(BAD_REQUEST,
             "'connection_id' field not present in the request body");
    }
    ...
    Optional<ConnectionData *> connection_data_opt =
            get_connection_data(connection_id);
    ...
    populate_ir_context_from_pool(&connection_data->connection->ir_ctx,
                                  shared_db_pool);

    // NOTE(oleh): We need to set this manually, since the IrContext is reset by
    // the populate_ir_context_from_pool call above.
    for (UZ db_idx = 0;
         db_idx < connection_data->connection->ir_ctx.database_schemas.count;
         ++db_idx)
    {
        if (connection_data->connection->ir_ctx.database_schemas[db_idx].name ==
            connection_data->connection->db->name)
        {
    ...
    bool ok = xmdb::compile_and_execute_source(
            &connection_data->temp_arena, connection_data->connection,
            source_sv, &query_results, &error);
    ...
    if (ok)
    {
        ...
        if (results_table_opt.has_value())
        {
            ...
            {
                ok::StringView column_name_sv =
                        results_table->columns_names()[i];
            ...
            for (UZ i = 0; i < results_table->columns_count(); ++i)
            {
                xmdb::SQL::ColumnType column_type =
                        results_table->columns_types()[i];
                const char *column_type_str =
                        xmdb::SQL::column_type_to_string(column_type);
            ...
            xmdb::DBTableStream *column_streams_ptr =
                    connection_data->temp_arena.alloc<xmdb::DBTableStream>(
                            results_table->columns_count());
            ok::Slice<xmdb::DBTableStream> column_streams = {
                    column_streams_ptr, results_table->columns_count()};
            for (UZ i = 0; i < results_table->columns_count(); ++i)
            {
                column_streams[i] = table_outlet.column_stream(
                        &connection_data->temp_arena, i);
            }
            ...
            for (UZ r = 0; r < results_table->rows_count(); ++r)
            {
                ...
                for (UZ i = 0; i < results_table->columns_count(); ++i)
                {
                    ...
                    case Value::Type::IMAGE_CHUNK:
                    {
                        ...
                        ok::String data_hex = xmdb::to_hex_string(
                                &connection_data->temp_arena, chunk->data);

                        WebJsonPutKey(WEB_SV_LIT("data"));
                        WebJsonPutString((web_string_view) {
                                .Items = (u8 *) data_hex.cstr(),
                                .Count = data_hex.count()});
    ...
    WebHttpContextAddHeader(ctx, WEB_SV_LIT("Content-Type"),
                            WEB_SV_LIT("application/json"));
    return HTTP_STATUS_OK;
}
...
DECLARE_HANDLER(get_db_objects_handler)
{
    ...
    ConnectionId connection_id;
    if (!WebJsonObjectGetU64(&json_obj, WEB_SV_LIT("connection_id"),
                             &connection_id))
    {
        FAIL(BAD_REQUEST,
             "'connection_id' field not present in the request body");
    ...
    ok::Slice<TableObject> table_objects =
            get_db_table_objects(&connection_data->temp_arena, db);
    ...
    for (UZ table_idx = 0; table_idx < table_objects.count; ++table_idx)
    {
        ...
        WebJsonPutString((web_string_view) {.Items = (u8 *) table.name.data,
        ...
        for (UZ col_idx = 0; col_idx < table.column_names.count; ++col_idx)
        {
            ...
            WebJsonPrepareArrayElement();
            WebJsonPutString((web_string_view) {.Items = (u8 *) col_name.data,
        ...
        for (UZ col_idx = 0; col_idx < table.column_types.count; ++col_idx)
        {
            ...
            const char *col_type_str =
                    xmdb::SQL::column_type_to_string(col_type);
            WebJsonPrepareArrayElement();
            WebJsonPutString((web_string_view) {.Items = (u8 *) col_type_str,
                                                .Count = strlen(col_type_str)});
        }
        WebJsonEndArray();
...
void run_http_server(U16 port)
{
    ...
    WebHttpServerAttachHandler(&server, "/get-db-objects",
                               get_db_objects_handler);
