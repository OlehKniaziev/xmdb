...
#define XMDB_DECLARE_BUILTIN_FUNCTION(name, ret, ...) static Result<ret, ErrorWithSourceLocation> builtin_##name(SourceLocation location, ok::Allocator *allocator, __VA_ARGS__)
...
XMDB_DECLARE_BUILTIN_FUNCTION(RGB, ImageChunk, U32 width, U32 height, ok::StringView hex_data) {
    ...
    if (hex_data.count != expected_hex_data_count) {
        FAIL_FMT(location, "the provided hex data has invalid length (wanted %lu, got %zu)",
                 expected_hex_data_count,
                 hex_data.count);
    }
...
StaticStorage *make_or_get_static_storage() {
    ...
    if (!init) {
        storage.builtin_functions = ok::Table<ok::StringView, void *>::alloc(&malloc_allocator);

#define X(fn_name, ret, ...) do { \
        XMDB_BUILTIN_FUNCTION_SIG_NAME(fn_ptr, ret, __VA_ARGS__) = builtin_##fn_name; \
        storage.builtin_functions.put(ok::StringView{#fn_name}, (void *) fn_ptr); \
        } while (false);
