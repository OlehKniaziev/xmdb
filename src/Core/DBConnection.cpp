...
static void execute_instruction(TypedCompiledQuery *query, UZ i,
                                QueryExecutionContext *ctx, DBConnection *conn)
{
    ...
    {
        Triple<String, U32, StringView> operands =
                operands_of_FetchColumn(&query->untyped, i);
    ...
    {
        Triple<String, StringView, TableSchema *> operands =
                operands_of_FetchTable(&query->untyped, i);
    ...
    {
        Tuple<StringView, TableSchema *> operands =
                operands_of_CreateTable(&query->untyped, i);
    ...
    {
        Triple<StringView, StringView, U32> operands =
                operands_of_AlterUserProperty(&query->untyped, i);
    ...
    {
        Triple<U32, U32, StringView> operands =
                operands_of_EmitColumn(&query->untyped, i);
    ...
    case IRInstructionOperator_EmitQuery:
    {
        ...
        ColumnType *column_types =
                ctx->allocator->alloc<ColumnType>(column_count);
    ...
    {
        Tuple<String, StringView> operands =
                operands_of_ConstString(&query->untyped, i);
    ...
    {
        Tuple<StringView, U32> operands =
                operands_of_InsertColumn(&query->untyped, i);
    ...
    { // NOTE(oleh): This instruction has no operands!
    ...
    {
        Triple<U32, U32, StringView> operands =
                operands_of_UpdateColumn(&query->untyped, i);
    ...
    {
        StringView database_name =
                operands_of_CreateDatabase(&query->untyped, i);
    ...
    case IRInstructionOperator_DropDatabase:
    {
        ...
        // NOTE(oleh): No databases exist, signal an error. Or should this have
    ...
    {
        Triple<String, StringView, S64> operands =
                operands_of_Call(&query->untyped, i);
...
template <>
DBValue *cpp_to_db_value<ImageChunk>(ok::Allocator *allocator,
                                     const ImageChunk &image_chunk)
{
...
template <>
DBValue *cpp_to_db_value<MediaSource>(ok::Allocator *allocator,
...
Result<Return, ErrorWithSourceLocation> call_impl(
        SourceLocation location, ok::Allocator *allocator,
        XMDB_BUILTIN_FUNCTION_SIG_NAME(fn_ptr, Return, Args...),
        ok::Array<DBValue *, sizeof...(Args)> raw_args, Seq<I...>)
{
    Slice<DBValue *> raw_args_slice = raw_args.slice();
    return fn_ptr(location, allocator,
                  extract<I, Args>(allocator, raw_args_slice)...);
}

template <typename Return, typename... Args>
Result<Return, ErrorWithSourceLocation> call(
        SourceLocation location, ok::Allocator *allocator,
        XMDB_BUILTIN_FUNCTION_SIG_NAME(fn_ptr, Return, Args...),
...
static void fill_column(DBRecord *record, DBTable *table, UZ column_index,
                        Value column_value)
{
    ...
    case Value::Type::STRING:
    {
        ...
        memcpy(&record->buffer[column_layout.offset],
               reinterpret_cast<U8 *>(&fs), sizeof(fs));
    ...
    case Value::Type::IMAGE_CHUNK:
    {
        ...
        image_state->file.seek_to(IMAGE_COLUMN_STATE_HEADER_SIZE +
                                  MAX_DISK_IMAGE_CHUNK_SIZE * new_chunk_index);

        UZ n_written = 0;
        // @Perf I'm not sure if it's better to do 2 writes of header and data
        // or memcpy potentially 64MiB pixel data. Need to benchmark this or
        // choose the strategy dynamically.
        ok::Optional<ok::File::WriteError> err = image_state->file.write(
                reinterpret_cast<U8 *>(&chunk), sizeof(chunk), &n_written);
        OK_VERIFY(!err);
        OK_VERIFY(n_written == sizeof(chunk));

        // NOTE(oleh): Need to seek manually here, since the 'write' method
        // seeks back to the original offset.
        image_state->file.seek_to(image_state->file.offset + sizeof(chunk));

        err = image_state->file.write(input_chunk->data.items,
                                      input_chunk->data.count, &n_written);
        OK_VERIFY(!err);
        OK_VERIFY(n_written == input_chunk->data.count);

        if (input_chunk->data.count < MAX_DISK_IMAGE_CHUNK_DATA_SIZE)
        {
            ok::Optional<ok::File::IOError> err = image_state->file.truncate(
                    IMAGE_COLUMN_STATE_HEADER_SIZE +
                    MAX_DISK_IMAGE_CHUNK_SIZE * (new_chunk_index + 1));
            OK_VERIFY(!err);
        }
    ...
    case Value::Type::MEDIA_SOURCE:
    {
        ...
        OK_VERIFY(table->columns_types()[column_index] ==
                  SQL::ColumnType::MEDIA);
        ...
        else
        {
            ...
            for (;;)
            {
                Result<UZ, ok::String> pull_res =
                        stream->pull(ok::temp_allocator());
                if (!pull_res.ok())
                {
...
// TODO(oleh): This should depend on the size of a single record for a given
// table, since we don't want to exceed some given memory usage quota.
constexpr UZ DB_RECORD_BUFFER_CAPACITY = 32;

static void flush_buffer(QueryExecutionContext *ctx, DBTable *table,
                         ok::Slice<DBRecord> record_buffer)
{
    ...
    // TODO(oleh): Instead of doing a write for each record, coalesce them as
    // much as possible.
    for (UZ i = 0; i < record_buffer.count; ++i)
    {
        ...
        state->file.seek_to(DB_STATE_HEADER_LENGTH +
                            record_size * (i + state->header.record_count));

        ok::Optional<ok::File::WriteError> write_err = state->file.write(
                reinterpret_cast<U8 *>(record.buffer), record_size, &n_written);
        OK_VERIFY(!write_err);
...
static void run_single_node(QueryExecutionContext *ctx, QueryGraph::Node *node)
{
    ...
    {
        QueryGraph::AlterUserNode *alter_node =
                static_cast<QueryGraph::AlterUserNode *>(node);
        ...
        {
            XMDB_FAIL_FMT(ctx,
                          "Could not find requested user with name '" OK_SV_FMT
                          "'",
                          OK_SV_ARG(user_name));
        }
        ...
        case QueryGraph::AlterUserNode::Property::PASSWORD:
        {
            ...
            DBTableStream password_value_stream = DBTableStream::from_value(
                    ctx->allocator, password_db_value);
    ...
    case QueryGraph::Node::Type::INSERT:
    {
        ...
        if (table->flags() & DBTable::F_PERSIST)
        {
            ...
            DBRecord *records_items =
                    ctx->allocator->alloc<DBRecord>(DB_RECORD_BUFFER_CAPACITY);
            Slice<DBRecord> records_buffer = {records_items,
                                              DB_RECORD_BUFFER_CAPACITY};
            UZ records_buffer_count = 0;

            for (UZ row_index = 0; row_index < rows_count; ++row_index)
            {
                DBRecord record =
                        db_record_create(ctx->allocator, table_layout);

                for (UZ column_index = 0;
                     column_index < insert_column_names.count; ++column_index)
                {
                    DBValue *column_db_value =
                            insert_column_values[row_index * insert_column_names
                                                                     .count +
                                                 column_index];
                    ok::Optional<Value> column_value =
                            poll(ctx->allocator, column_db_value);
                    OK_VERIFY(column_value);
                    fill_column(&record, table, column_index,
                                column_value.get());
                }
            ...
            flush_buffer(ctx, table,
                         records_buffer.slice(0, records_buffer_count));
        }
        else
        {
            OK_TODO();
#if OLD_CODE
            Slice<DBValue *> table_columns_values =
                    table->proxy_column_values();

            for (UZ row_idx = 0; row_idx < rows.count; ++row_idx)
            {
                UZ columns_to_insert_count =
                        rows_to_insert_columns_count[row_idx];
                StringView *columns_to_insert_names =
                        rows_to_insert_columns_names[row_idx];
                DBValue **columns_to_insert_values =
                        rows_to_insert_columns_values[row_idx];
                     ...
                     ++column_to_insert_idx)
                {
                    StringView column_name =
                            columns_to_insert_names[column_to_insert_idx];
                    for (UZ column_idx = 0; column_idx < table->columns_count();
                         ++column_idx)
                    {
                        if (table->columns_names()[column_idx] != column_name)
                            continue;

                        DBValue *old_value = table_columns_values[column_idx];
                        DBValue *value_to_insert =
                                columns_to_insert_values[column_idx];
                        DBValue *new_value = new (allocator)
                                ConcatDBValue{old_value, value_to_insert};
    ...
    {                                                                          \
        ...
        {                                                                      \
            void *raw_ptr = ctx->static_storage->builtin_functions             \
                                    .get(StringView{#name})                    \
                                    .get();                                    \
            auto fn_ptr = reinterpret_cast<XMDB_BUILTIN_FUNCTION_SIG(          \
                    ret, __VA_ARGS__)>(raw_ptr);                               \
            ...
            Result<ret, ErrorWithSourceLocation> call_result =                 \
                    call<ret, __VA_ARGS__>({}, ctx->allocator, fn_ptr,         \
                                           args_array);                        \
            if (!call_result.ok())                                             \
            {                                                                  \
                ErrorWithSourceLocation err = call_result.error();             \
                ok::String error_message =                                     \
                        SQL::format_error(ok::temp_allocator(), err.location,  \
                                          err.message.view());                 \
                XMDB_FAIL_FMT(ctx, "Failed to call builtin function '%s': %s", \
                              #name, error_message.cstr());                    \
            }                                                                  \
            else                                                               \
            {                                                                  \
                ret builtin_result = call_result.unwrap();                     \
                DBValue *builtin_value_result =                                \
                        cpp_to_db_value(ctx->allocator, builtin_result);       \
                DelayedDBValue *call_node_return_value =                       \
                        call_node->return_value();                             \
...
#undef X
    ...
    case QueryGraph::Node::Type::READ:  OK_TODO_MSG("READ");
    case QueryGraph::Node::Type::WRITE: OK_TODO_MSG("WRITE");
    case QueryGraph::Node::Type::ATOMIC:
    {
        XMDB_FIXME("Atomic nodes are not atomic at all currently");

        QueryGraph::AtomicNode *atomic_node =
                static_cast<QueryGraph::AtomicNode *>(node);
    ...
    case QueryGraph::Node::Type::WRITE_COLUMN:
    {
        ...
        DBTableStream col_stream =
                DBTableStream::from_value(ctx->allocator, col_value);
        ...
        for (UZ row_idx = 0; row_idx < rows_count; ++row_idx)
        {
            ...
            state->file.seek_to(DB_STATE_HEADER_LENGTH + record_size * row_idx +
                                col.offset);

            UZ n_written = 0;
            ok::Optional<ok::File::WriteError> err = state->file.write(
                    record.buffer + col.offset, col.size, &n_written);
            OK_VERIFY(!err);
...
void DBConnection::execute(TypedCompiledQuery *query, QueryResults *results)
{
    ...
    QueryExecutionContext *ctx =
            db_pool->rent_empty_execution_context(db, user);
