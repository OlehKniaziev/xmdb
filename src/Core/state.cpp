...
bool db_state_create(ok::File file, DBState *out_state)
{
    ...
    ok::Optional<ok::File::WriteError> write_err = file.write(
            reinterpret_cast<U8 *>(&header), sizeof(header), &n_written);
    if (write_err)
    {
        ok::String error_desc =
                ok::File::error_string(ok::temp_allocator(), write_err.get());
...
bool db_state_sync(DBState *state)
{
    ...
    UZ n_written = 0;
    ok::Optional<ok::File::WriteError> write_err =
            state->file.write(reinterpret_cast<U8 *>(&state->header),
                              sizeof(state->header), &n_written);
    if (write_err)
    {
        ok::String error_desc =
                ok::File::error_string(ok::temp_allocator(), write_err.get());
                                          ...
                                          ok::File file)
{
    ...
    ok::Optional<ok::File::ReadError> read_err =
            file.read(reinterpret_cast<U8 *>(&header), sizeof(header), &n_read);
    if (read_err)
    {
        ok::String error_desc =
                ok::File::error_string(allocator, read_err.get());
        return ok::String::format(allocator,
                                  "Failed to read state file header: %s",
                                  error_desc.cstr());
    }

    if (n_read != sizeof(header))
    {
        return ok::String::alloc(
                allocator, "State file is too small to contain a valid header");
    }
                                                      ...
                                                      ok::File file)
{
    ...
    if (read_err)
    {
        ok::String error_desc =
                ok::File::error_string(allocator, read_err.get());
        return ok::String::format(allocator,
                                  "Failed to read image state file header: %s",
                                  error_desc.cstr());
    }
    ...
    if (n_read != sizeof(header))
    {
        return ok::String::alloc(
                allocator,
                "Image state file is too small to contain a valid header");
    }

    if (header.magic != COLUMN_STATE_HEADER_MAGIC)
    {
        return ok::String::alloc(allocator,
                                 "Invalid image state file magic number");
    }
