...
DBPool::DBPool(ok::Allocator *allocator, Flags flags) :
    allocator{allocator}, flags{flags}, execution_contexts{nullptr}
{
    XMDB_FIXME("Replace this constructor by either a static method or a "
               "function, it can fail");
    ...
    {
        log::info("Found a catalog file in the current directory (" OK_SV_FMT
                  "), loading it",
                  OK_SV_ARG(catalog_file_name));

        Result<UZ, ok::String> cat_load_result =
                catalog_load(allocator, this, catalog_file_name);
    ...
    else
    {
        ...
        *admin = DBUser::admin();
        ...
        if (!ephemeral())
        {
            Result<UZ, ok::String> cat_save_result =
                    catalog_save(allocator, this, catalog_file_name);
            if (!cat_save_result.ok())
            {
                OK_PANIC_FMT("Failed to save a catalog to disk: %s",
                             cat_save_result.error().cstr());
            }
        }
    }
}
...
{
    for (DBDescriptor *curr_db = db_descriptors; curr_db != nullptr;
         curr_db = curr_db->next)
    {
        if (curr_db->name == db_name) return curr_db;
    }

    OK_PANIC_FMT("No database descriptor with name '" OK_SV_FMT "' found",
                 OK_SV_ARG(db_name));
}
                                                            ...
                                                            DBUser *user)
{
    ...
    ctx->emitted_columns =
            ok::MultiList<StringView, DBValue *, DBTable *>::alloc(allocator);
    ctx->insert_column_names = ok::List<StringView>::alloc(allocator);
    ctx->insert_column_values = ok::List<DBValue *>::alloc(allocator);
    ctx->rows_to_insert =
            ok::List<ok::Pair<Slice<StringView>, Slice<DBValue *>>>::alloc(
                    allocator);
    ctx->columns_to_update =
            ok::MultiList<StringView, DBValue *>::alloc(allocator);
