...
struct BTreeState
{
    static ok::Optional<ok::String> create(ok::Allocator *allocator,
                                           ok::File state_file,
                                           BTreeState **out_state)
    {
        ...
        else
        {
            ...
            if (read_err)
                return ok::File::error_string(allocator, read_err.value);
    ...
    ok::Optional<ok::String> reset()
    {
        ...
        memcpy(temp_buf + BTREE_HEADER_LENGTH, this->root->disk,
               BTREE_PAGE_SIZE);
        ...
        if (write_err)
            return ok::File::error_string(allocator, write_err.value);
    ...
    Node *load_node_from_disk(Node *parent, U64 child_index)
    {
        ...
        auto read_err = state_file.read(reinterpret_cast<U8 *>(node->disk),
                                        BTREE_PAGE_SIZE, &num_read);
        if (read_err) OK_TODO();
    ...
    void save_node_to_disk(Node *node)
    {
        ...
        UZ n_written = 0;
        auto write_err = state_file.write(reinterpret_cast<U8 *>(node->disk),
                                          BTREE_PAGE_SIZE, &n_written);
    ...
    void save_header_to_disk()
    {
        state_file.seek_start();
        UZ n_written = 0;
        auto write_err = state_file.write(reinterpret_cast<U8 *>(header),
                                          BTREE_HEADER_LENGTH, &n_written);
        if (write_err) OK_TODO();
        ...
        if (!full_child->is_leaf())
        {
            for (U64 i = 0; i < BTREE_ORDER; ++i)
            {
                new_child->children()[i] =
                        full_child->children()[BTREE_ORDER + i];
            }
        }

        full_child->set_keys_count(BTREE_ORDER - 1);

        // NOTE(oleh): Access of children here cannot be out-of-bounds, since
        // the parent node is not full.
        for (S64 i = (S64) parent->keys_count(); i > (S64) full_child_id; --i)
        {
            parent->children()[i + 1] = parent->children()[i];
        }

        for (S64 i = (S64) parent->keys_count() - 1;
             i > (S64) full_child_id - 1; --i)
        {
...
ok::Optional<ok::File::OpenError> BTreeIndex::create(
...
{
    BTreeIndex tree{};
    auto error_string = BTreeState::create(allocator, state_file,
                                           (BTreeState **) &tree.pImpl);
