...
#define X(p, _unused)                                                          \
    ...
    {                                                                          \
        ...
        {                                                                      \
            XMDB_FAIL_FMT(ctx,                                                 \
                          "user " OK_SV_FMT " does not have " #p               \
                          " permissions",                                      \
...
void QueryExecutionContext::sync_state()
{
    ...
    if (!result.ok())
    {
        XMDB_FAIL_FMT(this, "Failed to write database state to disk: %s",
                      result.error().cstr());
    }
}
...
DBValue *QueryExecutionContext::fetch_column(DBTable *table,
...
void QueryExecutionContext::create_table(StringView table_name,
                                         SQL::TableSchema *table_schema)
{
    ...
    for (UZ tab_idx = 0; tab_idx < current_db->tables.count; ++tab_idx)
    {
        ...
        if (table->name() == table_name)
        {
            XMDB_FAIL_FMT(this, "table '" OK_SV_FMT "' already exists",
    ...
    ok::StringView *column_names_ptr =
            allocator->alloc<ok::StringView>(columns_count);
    ...
    DBTable *new_table = current_db->create_new_table(
            allocator, table_name, table_flags, column_types.count,
            column_names, column_types.slice());

    if (new_table == nullptr)
    {
        XMDB_FAIL_FMT(this,
                      "Failed to create table '" OK_SV_FMT
...
void QueryExecutionContext::emit_column(DBTable *table, DBValue *column_value,
                                        StringView column_name)
{
    emitted_columns.push(column_name, column_value, table);
}

DBTable *QueryExecutionContext::emit_query(U32 column_count,
                                           SQL::ColumnType *column_types)
{
    const StringView *column_names_ptr =
            emitted_columns.get_items<StringView>();
    ...
    memcpy(column_names, column_names_ptr,
           column_count * sizeof(*column_names));
    memcpy(column_values, column_values_ptr,
           column_count * sizeof(*column_values));
    ...
    QueryGraph::EmitQueryNode *query_node = query_graph.emit_query(
            allocator, queried_tables_slice, column_names_slice,
            column_values_slice, column_types_slice);
    DBTable *table = query_node->table();
    last_emitted_query = table;
    return table;
}

void QueryExecutionContext::insert_column(StringView column_name,
                                          DBValue *column_value)
{
    CHECK_WRITE(this);
...
void QueryExecutionContext::insert_row()
{
    Slice<StringView> column_names =
            insert_column_names.copy(allocator).slice();
    Slice<DBValue *> column_values =
            insert_column_values.copy(allocator).slice();
...
void QueryExecutionContext::commit_insert(DBTable *table)
{
    ...
    query_graph.insert(table, insert_column_names.slice(),
                       insert_column_values.slice(), rows_to_insert.count);

    rows_to_insert.count = 0;
}

void QueryExecutionContext::update_column(DBTable *table,
                                          StringView column_name,
                                          DBValue *column_value)
{
...
void QueryExecutionContext::commit_update()
{
    ...
    StringView *columns_to_update_names =
            columns_to_update.get_items<StringView>();
    DBValue **columns_to_update_values =
            columns_to_update.get_items<DBValue *>();
...
void QueryExecutionContext::create_user(StringView name)
{
    ...
    for (DBUser *user = current_db->users; user != nullptr; user = user->next)
    {
        if (user->name == name)
        {
            XMDB_FAIL_FMT(this, "user '" OK_SV_FMT "' already exists",
                          OK_SV_ARG(name));
        }
    }
...
void QueryExecutionContext::alter_user_property(StringView user_name,
                                                ...
                                                DBValue *value)
{
    ...
    if (property_name == "PASSWORD"_sv)
    {
        if (value->type() != SQL::TYPE_STRING)
            XMDB_FAIL_FMT(this,
                          "expected user password to be of type string, got "
                          "type '%s' instead",
                          SQL::type_name(value->type()));

        auto *alter_node = new (allocator) QueryGraph::AlterUserNode{
                user_name, QueryGraph::AlterUserNode::Property::PASSWORD,
                value};
    ...
    {
        XMDB_FAIL_FMT(this,
                      "user '" OK_SV_FMT "' does not have property '" OK_SV_FMT
                      "'",
                      OK_SV_ARG(user_name), OK_SV_ARG(property_name));
    }
}
...
DBValue *QueryExecutionContext::call(ok::StringView fn_name, U64 arg_count)
{
    ...
    QueryGraph::CallNode *call_node =
            query_graph.call(allocator, fn_name, args);
