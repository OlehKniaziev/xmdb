...
public:
    ...
    {
    public:
        ReadNode(U64 offset, U64 size) :
            Node{Type::READ}, m_size{size}, m_offset{offset}
        {
        }
    ...
    {
    public:
        WriteNode(U64 offset, U64 size) :
            Node{Type::WRITE}, m_size{size}, m_offset{offset}
        {
        }
    ...
    public:
        ...
        explicit AlterUserNode(ok::StringView user_name, Property property,
                               DBValue *property_value) :
            Node{Type::ALTER_USER},
    ...
    /**
     * @brief Node representing a collection of operations that must be
    ...
    {
    public:
        WriteColumnNode(DBTable *table, UZ idx, DBValue *value) :
            Node{Type::WRITE_COLUMN}, m_table{table}, m_idx{idx}, m_value{value}
        {
        }
    ...
    {
    public:
        CallNode(Allocator *allocator, StringView fn_name,
                 Slice<DBValue *> args) :
            Node{Type::CALL},
            m_fn_name{fn_name},
            m_args{args},
    ...
    public:
        ...
        InsertNode(DBTable *table, Slice<StringView> column_names,
                   Slice<DBValue *> column_values, UZ rows_count) :
            Node{Type::INSERT},
            m_table{table},
    ...
    {
    public:
        EmitQueryNode(Allocator *allocator, Slice<DBTable *> queried_tables,
                      Slice<StringView> column_names,
                      Slice<DBValue *> column_values,
                      Slice<SQL::ColumnType> column_types) :
            Node{Type::EMIT_QUERY},
            m_queried_tables{queried_tables},
            m_column_values{column_values},
            m_emitted_table{new(allocator) DBTable{
                    allocator, DBTable::F_ANON | DBTable::F_PROXY, ""_sv,
                    column_values.count, column_names.items,
                    column_types.items}}
        {
        }
    ...
    {
    public:
        explicit DeleteTableNode(DBTable *table) :
            Node{Type::DELETE_TABLE}, m_table{table}
        {
        }
     ...
     * @return The added node.
     */
    CallNode *call(ok::Allocator *allocator, ok::StringView fn_name,
                   Slice<DBValue *> args)
    {
    ...
    EmitQueryNode *emit_query(Allocator *allocator,
                              ...
                              Slice<SQL::ColumnType> column_types)
    {
        return add_generic_node<EmitQueryNode>(allocator, queried_tables,
     ...
     * @return The added node.
     */
    InsertNode *insert(DBTable *table, Slice<StringView> names,
                       Slice<DBValue *> values, UZ rows_count)
    {
