...
{
    ok::String message = xmdb::SQL::format_error(
            ok::temp_allocator(), error.location, error.message.view());
    printf("%s: %s\n", why, message.cstr());
    exit(1);
}
...
int main(int argc, char **argv)
{
    ...
    arg_parser.boolean("untyped", &untyped,
                       "Whether to type check the compiled query");
    arg_parser.string("tables", &tables_str,
                      "List of comma-separated tables to define for the "
                      "semantic analysis. Useful with the '-untyped' flag",
                      "");
    ...
    if (!arg_parser.parse())
    {
        ...
        arg_parser.help();
        xmdb::dief("\nFailed to parse command line arguments: " OK_SV_FMT,
    ...
    for (UZ table_idx = 0; table_idx < tables.count; ++table_idx)
    {
        ...
        ir_ctx.alloc_table_schema(ir_ctx.active_db_id,
                                  table_name.to_string(&arena), false);
    }

    bool result = xmdb::SQL::ir_compile_query(&q, &ir_ctx, &compiled_query);
    if (!result)
    {
