...
constexpr ok::StringView PLUG_GET_LAST_ERROR_NAME =
        "xmdb_plugin_get_last_error"_sv;

Result<Plugin *, ok::String> Plugin::load(ok::Allocator *allocator,
                                          const NativeLibrary *native_lib)
{
    ok::Optional<NativeSymbol> install_sym =
            native_lib->get_symbol(PLUG_INSTALL_NAME);
    if (!install_sym.has_value())
    {
        return ok::String::format(allocator,
                                  "The native library does not export the "
                                  "'install' hook '" OK_SV_FMT "'",
                                  OK_SV_ARG(PLUG_INSTALL_NAME));
    }

    InstallHook install_hook = install_sym.get().cast<InstallHook>();

    ok::Optional<NativeSymbol> get_last_error_sym =
            native_lib->get_symbol(PLUG_GET_LAST_ERROR_NAME);
    GetLastErrorHook get_last_error_hook =
            get_last_error_sym.or_else({}).cast<GetLastErrorHook>();

    ok::Optional<NativeSymbol> load_sym =
            native_lib->get_symbol(PLUG_LOAD_NAME);
    if (!load_sym)
    {
    ...
    if (!ok)
    {
        ...
        if (get_last_error_hook != nullptr)
            error = get_last_error_hook(nullptr);

        if (error != nullptr)
        {
            return ok::String::format(
                    allocator,
                    "Failed to call the 'load' hook of the plugin: %s", error);
        }
    }

    ok::Optional<NativeSymbol> unload_sym =
            native_lib->get_symbol(PLUG_UNLOAD_NAME);
...
void Plugin::install(ok::StringView name)
{
    ...
    m_install_hook(m_plugin_state, cstr_name, &caps_names, &caps_syms,
                   &caps_count);
...
Result<PluginCapability, ok::String> Plugin::get_capability(
        ok::StringView name) const
{
    if (!m_installed)
    {
        return ok::String::format(
                m_allocator,
                "cannot get capability '" OK_SV_FMT
                "' as the plugin's 'install' hook has not been called yet",
                OK_SV_ARG(name));
    }

    for (UZ cap_idx = 0; cap_idx < m_capabilities.count; ++cap_idx)
    {
        const ok::Pair<ok::StringView, NativeSymbol> *cap =
                &m_capabilities[cap_idx];
        if (cap->a == name) return PluginCapability{cap->a, cap->b};
    }

    return ok::String::format(m_allocator,
                              "capability '" OK_SV_FMT "' was not found",
                              OK_SV_ARG(name));
}
