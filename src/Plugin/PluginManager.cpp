...
{
PluginManager::PluginManager(ok::Allocator *allocator) :
...
Result<Plugin *, ok::String> PluginManager::get_or_load(ok::StringView path)
{
    ...
    Result<NativeLibrary, ok::String> lib_res =
            NativeLibrary::load(m_allocator, path);
    if (!lib_res.ok())
    {
        return ok::String::format(
                m_allocator,
                "Failed to load a native library from '" OK_SV_FMT "': %s",
                OK_SV_ARG(path), lib_res.error().cstr());
    }
    ...
    if (!plug_res.ok())
    {
        return ok::String::format(
                m_allocator,
                "Failed to load a plugin from a native library '" OK_SV_FMT
                "': %s",
                OK_SV_ARG(path), plug_res.error().cstr());
    }
